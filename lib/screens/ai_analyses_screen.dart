...
class AiAnalysesScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Yapay Zeka Analizlerim'),
        elevation: 1,
      ),
      body: Consumer<JournalProvider>(
        builder: (context, journalProvider, child) {
          ...
          // Sadece anxiety ve thought entries'leri al
          final aiEntries = journalProvider.entries
              .where((entry) => entry.type == 'anxiety' || entry.type == 'thought')
              .toList();
          ...
          if (aiEntries.isEmpty) {
            return Center(
              child: Column(
                ...
                children: [
                  Icon(FlutterRemix.robot_line, size: 60, color: Colors.grey[600]),
                  ...
                  Padding(
                    ...
                    child: Text(
                      'Anksiyete kaydı veya düşünce kaydı eklediğinizde burada görünecek.',
  ...
  Widget _buildAnxietyEntryCard(BuildContext context, JournalEntry entry) {
    ...
    return Card(
      ...
      child: ListTile(
        ...
        subtitle: Text(
          (data['tetikleyici'] as String?)?.isNotEmpty == true
              ? data['tetikleyici']
              : 'Tetikleyici belirtilmemiş',
          maxLines: 1,
